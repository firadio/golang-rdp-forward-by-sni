...
**安全机制**：
- **TLS连接**：协商（0x03包）→ TLS升级（0x16包）→ SNI提取与检查
- **非TLS连接**：协商（0x03包）→ MCS层解析 → 客户端名称提取与检查（仅限未加密连接）
- 白名单可以包含：TLS的SNI域名/IP、RDP客户端计算机名
- 注意：非TLS的RDP客户端识别依赖于未加密的连接，如果启用了RDP标准加密将无法提取客户端信息
